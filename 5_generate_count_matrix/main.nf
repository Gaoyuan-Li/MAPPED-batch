...
workflow {
    ...
    """
        ...
        .map { row ->
            // The downloaded files are named with the full id (e.g., SRX22235850_SRR26532638_1.fastq.gz)
            // but the samplesheet paths don't include the experiment prefix
            def fastq_dir = "${params.outdir}/seqFiles/fastq"
            def reads = [file("${fastq_dir}/${row.id}_1.fastq.gz")]
            if (row.fastq_2 && row.fastq_2.trim()) {
                reads.add(file("${fastq_dir}/${row.id}_2.fastq.gz"))
            }
            tuple(row.id, reads)
        }
    ...
    multiqc_json_ch = multiqc.json
        ...
        .ifEmpty { 
            ...
            def multiqc_dir = file("${params.outdir}/multiqc")
            multiqc_dir.mkdirs()
            def empty_json = file("${params.outdir}/multiqc/empty_multiqc.json")
            empty_json.text = '{}'
            empty_json
        }
    ...
    passed_ch = parse_qc_result.passlist
        ...
        .ifEmpty { 
            ...
            def multiqc_dir = file("${params.outdir}/multiqc")
            multiqc_dir.mkdirs()
            def empty_passlist = file("${params.outdir}/multiqc/empty_passlist.txt")
            empty_passlist.text = ''
            empty_passlist
        }
